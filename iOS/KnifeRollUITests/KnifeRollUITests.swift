...
final class KnifeRollUITests: XCTestCase {
    ...
    func testAddItemFlow() {
        ...
        nameField.tap()
        nameField.typeText("UI Test Knife")
        app.buttons["itemSaveButton"].tap()
        XCTAssertTrue(app.staticTexts["UI Test Knife"].waitForExistence(timeout: 2))
    }
    ...
    func testFreeLimitTriggersPaywall() {
        for i in 0..<10 {
            ...
            app.buttons["addItemButton"].tap()
            if app.staticTexts["KnifeRoll Pro"].waitForExistence(timeout: 1) {
                break
            }
            let nameField = app.textFields["itemNameField"]
